...
let _layer__name = 'dibit1_pt__raster';
let _layer__label = 'DIBIT1 (PT) Blueprint';
let _layer__ready = false;

let _first_frameUrl = 'https://cityplanner.biz/data/indoor_map/dibit1_pt_modified_4326.png';

function timeout(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

export async function layer__dibit1_pt__raster__init(){

  if(mymap__isReady() == false){
    await timeout(500);
    return await layer__dibit1_pt__raster__init();
    // return false;
  }

  _mymap = mymap__get();

  dibit1_pt__raster__source__add();

}

function dibit1_pt__raster__source__add(){
  ...
  _mymap.addSource(`${_layer__name}`, {
    ...
    'url': _first_frameUrl,
    'coordinates': [
      [9.2661728799999992,45.5074576029999989],   // Top-left
      [9.2673642019999996,45.5074576029999989],    // Top-right
      [9.2673642019999996,45.5061964519999975],   // Bottom-right
      [9.2661728799999992,45.5061964519999975]   // Bottom-left
    ]
  });
