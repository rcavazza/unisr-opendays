...
let _layer__name = "dibit2_1p";
let _layer__label = "DIBIT2 (1P)";
...
export async function layer__dibit2_1p__init() {
  if (mymap__isReady() == false) {
    await timeout(500);
    return await layer__dibit2_1p__init();
    // return false;
  }

  _mymap = mymap__get();

  dibit2_1p__source__add();
}

async function dibit2_1p__source__add() {
  let _r = await Promise.all([
    _axios("pg_dibit2_1p"),
    _axios("pg_dibit2_1p_walls"),
    _axios("pg_dibit2_1p_amenity"),
  ]);
...
function _toc(_layer__name, visibility = "visible") {
  ...
  $(`.tocbox__check__btn[toc_token="${_layer__name}"]`).on(
    ...
    function () {
      ...
      floors_change("bibit2", "Bibit2 1P");
    }
  );
}
