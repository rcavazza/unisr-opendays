...
let _layer__name = "dibit1_1s";
let _layer__label = "DIBIT1 (1S)";
...
export async function layer__dibit1_1s__init() {
  if (mymap__isReady() == false) {
    await timeout(500);
    return await layer__dibit1_1s__init();
    // return false;
  }

  _mymap = mymap__get();

  dibit1_1s__source__add();
}

async function dibit1_1s__source__add() {
  let _r = await Promise.all([
    _axios("pg_dibit1_1s"),
    _axios("pg_dibit1_1s_walls"),
    _axios("pg_dibit1_1s_amenity"),
  ]);
...
function _toc(_layer__name, visibility = "visible") {
  ...
  $(`.tocbox__check__btn[toc_token="${_layer__name}"]`).on(
    "click",
    function () {
      console.log("toc", $(this).attr("toc_token"));
      ...
      floors_change("bibit1", "Bibit1 1S");
    }
  );
}
