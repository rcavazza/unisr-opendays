...
let _layer__name = "dibit2_pt";
let _layer__label = "DIBIT2 (PT)";
...
export async function layer__dibit2_pt__init() {
  if (mymap__isReady() == false) {
    await timeout(500);
    return await layer__dibit2_pt__init();
    // return false;
  }

  _mymap = mymap__get();

  dibit2_pt__source__add();
}

async function dibit2_pt__source__add() {
  let _r = await Promise.all([
    _axios("pg_dibit2_pt"),
    _axios("pg_dibit2_pt_walls"),
    _axios("pg_dibit2_pt_amenity"),
  ]);
...
function _toc(_layer__name, visibility = "visible") {
  ...
  $(`.tocbox__check__btn[toc_token="${_layer__name}"]`).on(
    ...
    function () {
      ...
      floors_change("bibit2", "Bibit2 PT");
    }
  );
}
