...
let _layer__name = "dibit1_pt";
let _layer__label = "DIBIT1 (PT)";
...
export async function layer__dibit1_pt__init() {
  if (mymap__isReady() == false) {
    await timeout(500);
    return await layer__dibit1_pt__init();
    // return false;
  }

  _mymap = mymap__get();

  dibit1_pt__source__add();
}

async function dibit1_pt__source__add() {
  let _r = await Promise.all([
    _axios("pg_dibit1_pt"),
    _axios("pg_dibit1_pt_walls"),
    _axios("pg_dibit1_pt_amenity"),
  ]);
...
function _toc(_layer__name, visibility = "visible") {
  ...
  $(`.tocbox__check__btn[toc_token="${_layer__name}"]`).on(
    ...
    function () {
      ...
      [`dibit1_1p`, `dibit1_pt`, `dibit1_1s`, `dibit1_2s`].forEach(
        (_this__name) => {
          if (_this__name != $(this).attr("toc_token")) {
            ...
            // _mymap.setLayoutProperty(`id-${_this__name}__line`, 'visibility', `none`);
            _mymap.setLayoutProperty(
      ...
      floors_change("bibit1", "Bibit1 PT");
    }
  );
  ...
  //   [`dibit1_1p`,`dibit1_pt`,`dibit1_1s`,`dibit1_2s`].forEach((_this__name) => {
  //     if(_this__name != _layer__name){
  //       _mymap.setLayoutProperty(`id-${_this__name}__polygon`, 'visibility', `none`);
  //       // _mymap.setLayoutProperty(`id-${_this__name}__line`, 'visibility', `none`);
  //       _mymap.setLayoutProperty(`id-${_this__name}__walls`, 'visibility', `none`);
  //       _mymap.setLayoutProperty(`id-${_this__name}__amenity`, 'visibility', `none`);
  //     }
  //   });

  //   _mymap.setLayoutProperty(`id-${_layer__name}__polygon`, 'visibility', visibility);
  //   // _mymap.setLayoutProperty(`id-${_layer__name}__line`, 'visibility', visibility);
  //   _mymap.setLayoutProperty(`id-${_layer__name}__walls`, 'visibility', visibility);
  //   _mymap.setLayoutProperty(`id-${_layer__name}__amenity`, 'visibility', visibility);
