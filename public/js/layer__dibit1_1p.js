...
let _layer__name = "dibit1_1p";
let _layer__label = "DIBIT1 (1P)";
...
export async function layer__dibit1_1p__init() {
  if (mymap__isReady() == false) {
    await timeout(500);
    return await layer__dibit1_1p__init();
    // return false;
  }

  _mymap = mymap__get();

  dibit1_1p__source__add();
}

async function dibit1_1p__source__add() {
  let _r = await Promise.all([
    _axios("pg_dibit1_1p"),
    _axios("pg_dibit1_1p_walls"),
    _axios("pg_dibit1_1p_amenity"),
  ]);
  ...
  //   `id-ghost-6`
  // );
  let _paint = { "fill-color": layer__styles__unit__get() };
  // _paint['fill-extrusion-height'] = 3;
  // _paint['fill-extrusion-base'] = 3;
  // _paint['fill-extrusion-opacity'] = 1;
  _mymap.addLayer(
    {
      id: `id-${_layer__name}__polygon`, // Layer ID
      type: "fill",
...
function _toc(_layer__name, visibility = "visible") {
  ...
  $(`.tocbox__check__btn[toc_token="${_layer__name}"]`).on(
    ...
    function () {
      ...
      floors_change("bibit1", "Bibit1 1P");
    }
  );
}
