 ...
 * con gestione speciale per il file del database SQLite
 */
...
function pushChangesToMain() {
    try {
        ...
        // Ignora temporaneamente il file del database
        console.log(`Ignoro temporaneamente il file del database ${DB_FILE}...`);
        try {
            // Aggiungi il file al .gitignore temporaneamente
            execSync(`echo "${DB_FILE}" >> .gitignore`, { stdio: 'inherit' });
            // Rimuovi il file dal tracking
            execSync(`git rm --cached ${DB_FILE}`, { stdio: 'inherit' });
            execSync('git add .gitignore', { stdio: 'inherit' });
            execSync('git commit -m "Ignora temporaneamente il database"', { stdio: 'inherit' });
            console.log('Database ignorato temporaneamente');
        } catch (error) {
            console.log(`Errore nell'ignorare il database: ${error.message}`);
        }
        ...
        // Rimuovi il database dal .gitignore
        console.log('Rimuovo il database dal .gitignore...');
        try {
            // Leggi il .gitignore e rimuovi la riga con il DB_FILE
            const fs = require('fs');
            const gitignore = fs.readFileSync('.gitignore', 'utf8');
            const newGitignore = gitignore.split('\n').filter(line => line.trim() !== DB_FILE).join('\n');
            fs.writeFileSync('.gitignore', newGitignore);
            execSync('git add .gitignore', { stdio: 'inherit' });
            execSync('git commit -m "Ripristina tracking del database"', { stdio: 'inherit' });
            console.log('Database rimosso dal .gitignore');
        } catch (error) {
            console.log(`Errore nel rimuovere il database dal .gitignore: ${error.message}`);
        }
